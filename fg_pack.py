    ...
    all_data.append(
        {
            ...
            "id": (
                rec.get("sale_order_line")[0]
                if isinstance(rec.get("sale_order_line"), list)
                else (rec.get("sale_order_line") or "")
            ),
...
if not df.empty:
    ...
    df = df.groupby(["action_date", "oa", "company", "item", "id"], as_index=False)[
        ["qty", "pack_value"]
    ].sum()
